/* *Copyright(C), Haijie Lee. 2019. AllRightsReserved.
	*FileName:  Coap流处理 模块源文件.
	*Author:  Haijie Lee.
	*Version:  1.5.
	*Date:  2019.01.22.
	*Description:  实现Coap流的处理过程。
							本模块依赖 <common>实现字节序转换、
							依赖<iot_user>提供的用户通信数据内容、
							依赖<BCxx_Module>、实现coap数据流与NB模组的交换过程。
...
uint16_t Get_MessagePeriod( uint8_t message_id )
{
	...
	for(i=0; i<MESSAGE_MAX; i++) {
		if(message_id == gl_Message_table[i].message_type)
			break;
	}
	if(i == MESSAGE_MAX) {
		LOGE("POST_IOT_Message_Period error: ‘message_type’ not found.\n");
